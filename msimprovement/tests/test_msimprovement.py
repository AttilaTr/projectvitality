...
class TestHome(TestBase):
    def test_get_improvement_indoor(self):
        ...
        for case in test_cases:
            response = self.client.post(url_for('get_improvement_indoor'), data=case[0])
            self.assertEqual(response.data.decode('utf-8'), case[1])